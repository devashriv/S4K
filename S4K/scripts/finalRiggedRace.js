...
function animate() {
    ...
    if (currentTime < 2)
    {
        cube1.position.x -= 60 * frameTime;
        cube1.position.y = Math.cos( verticalAngle ) * 1.5 + 2;
    }
    else if (currentTime >= 2 && currentTime < 3)
    {
        cube1.position.x += (currentTime - 1.0) * (currentTime - 1.0);
        cube1.position.y = Math.cos( verticalAngle ) * 1.5 + 2;
    }
    else
    {
        cube1.position.y = Math.cos( verticalAngle ) * 1.5 + 2;
        ducky1.position.x = xoff + v1_0 * Math.log(1 + alp1 * v1_0 * (currentTime-3));// Math.cos( horizontalAngle ) * 4;
        ducky2.position.x = xoff + v2_0 * Math.log(1 + alp2 * v2_0 * (currentTime-3));// Math.cos( horizontalAngle ) * 4;
        ducky3.position.x = xoff + v3_0 * Math.log(1 + alp3 * v3_0 * (currentTime-3));// Math.cos( horizontalAngle ) * 4;
    }
